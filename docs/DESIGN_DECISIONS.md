...
|---|---|---|
| **Calm default** | General users | Off-white (#F7F7F2) reduces screen glare. Muted teal accent is non-aggressive. No pure white as pure white creates harsh contrast under stress. |
...
The model is explicitly instructed to split overlong actions into two tasks rather than stretch the word limit. Tasks are not hard-truncated in Python because truncation corrupts meaning. The prompt guidance is the enforcement layer.
...
File attachment supports .txt, .pdf, .docx, and screenshots (.png, .jpg, .jpeg). All files are extracted in memory and nothing is stored. All extracted text passes through the full content screening stack before reaching the user or the AI pipeline.
...
All of this runs before any text is returned to the frontend. Files are never stored but extracted in memory only.

---

## Medical Safety Hardening

Medical content receives a stricter pipeline. All of the following are enforced in Python inside `validate_response()`:
...
- **Medical badge blocked:** Medical content never shows "CLEAR", it is enforced in `renderResult()`
- **Persistent disclaimer bar:** Stays visible across all phases and never cleared by phase transitions
...
- Motor difficulties make selecting and copying text from a PDF painful
- Low digital literacy means multi-step copy/paste workflows are confusing
- Some documents arrive as images, photos of letters and screenshots of instructions

The upload pipeline supports .txt, .pdf, .docx, and screenshots (.png, .jpg, .jpeg). All files are extracted in memory and nothing is stored. All extracted text passes through the full content screening stack before reaching the user or the AI pipeline. The file attachment button is deliberately sized and styled to be visible and easy to click ensuring larger padding, legible font and a distinct border, while remaining calm and consistent with the ClearStep aesthetic.

The connection between upload and calendar reminders is intentional: a user who cannot easily copy a medical instruction should also not have to manually create a reminder. The full flow including the attach, process, step through, and remind is designed to require as little friction as possible.
...
This is a direct response to the challenge brief, not an omission of it. Timers are a common pattern in productivity tools. For neurodiverse users, they are often harmful.
...
For neurodiverse users, false reassurance can be actively misleading. If a user is completing a task incorrectly, e.g, misreading a medical instruction or missing a deadline field in a government form, being told they are "doing great" undermines their ability to course-correct. It also creates a false sense of completion that can have real consequences.
