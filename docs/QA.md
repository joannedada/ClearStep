...
> **5. Azure**
> 10 Azure services + Microsoft Foundry: each with a specific architectural role. Not decorative integrations.
...
The two problems are different in kind. "Is this safe?" requires risk assessment and signal detection. "Make it simple" requires extraction, sequencing, and cognitive pacing. Merging them into one flow would compromise both. Judges evaluate Mode 1 and Mode 2 as separate, complementary capabilities.
...
The system was tested across 14+ attack vectors including prompt injection, schema manipulation, file upload abuse, and safety bypass attempts. **Defences are enforced before, during, and after model execution.** Results are documented in `docs/SECURITY.md`.
...
Reading level is inferred from behavioral history across sessions. Every manual reading level change is logged to Cosmos DB. On return, if the stored history contains 3 or more entries and one level appears at least twice across the full history, that level is auto-applied before the user touches anything, with a visible toast so they know it happened. Language detection makes multilingual support invisible, no selector, no configuration. Accessibility palette persists across sessions and devices. The system does not require the user to understand how it adapts, it just does, and it shows its work.

**Why did you build file upload instead of just text input?**
The users who benefit most from ClearStep often have the hardest time using text input alone. Motor difficulties make copy/paste from a PDF painful. Low digital literacy makes multi-step workflows confusing. File upload supports .txt, .pdf, and .docx, all extracted in memory, screened through the full safety pipeline, and placed directly into the analysis flow. Each extracted step can be turned into a calendar reminder, so the user can act without holding the full document in memory.

**The challenge mentions time-boxed tasks. Why no timers?**
Deliberate decision. For users already experiencing cognitive overload, countdowns introduce urgency and pressure, which increases cognitive load instead of reducing it. ClearStep supports pacing through user-controlled step progression, batched task delivery (5 at a time), and optional user-initiated reminders. The user moves forward when they are ready, not when a timer tells them to.

**Why no encouragement messages like "You're doing great"?**
AI does not have full context of the user's situation. ClearStep sees the text that was pasted, it does not know whether the user has understood the steps, has the resources to complete them, or whether their situation has changed. For neurodiverse users, false reassurance can be actively misleading, especially if a task is being done incorrectly. ClearStep prioritises clarity and trust over artificial positivity. Results are calm and factual. The app never tells users what they want to hear.
...
Session history for returning users, and a browser extension for "Is This Safe?" directly from email clients.
